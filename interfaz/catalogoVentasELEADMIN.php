...
//se mostraran los productos mientras correspondan a la categoria filtrada y que su disponibilidad sea mayor a 0
        ...
        $sql="SELECT * FROM catalogo_venta where categoria='Eléctrico' and estatus>=1 ORDER BY nombre_producto_v ASC";
...
echo '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><hr class="hr2"></div>';
        ...
        if(isset($_GET['ini'])){
            $ini=$_GET['ini'];
            $fin=$_GET['fin'];
            $sql="SELECT * FROM catalogo_venta where categoria='Eléctrico' and estatus>=1 ORDER BY nombre_producto_v ASC  LIMIT 20 offset $ini";
        }else{
            $sql="SELECT * FROM catalogo_venta where categoria='Eléctrico' and estatus>=1 ORDER BY nombre_producto_v ASC  LIMIT 20 offset 0";
        }
...
//aqui se muestra la informacion correspondiente para el catalogo como imagen- precio- disponibiidad- y boton para ver la descripcion de cada producto
        ...
        for($c=1;$c<=$pag;$c++){
            $ini=$c*20-20;
            $fin=$ini+19;
            echo ' <a href="catalogoVentasELEADMIN.php?ini='.$ini.'&fin='.$fin.'#ir">'.$c.'</a>&nbsp;&nbsp;';
        }
        echo '</div>';
            ?>
