...
while ($row = pg_fetch_array($query)) { //aqui cargo las variables correspondientes al array del producto
    ...
    if ($cantidadApartada > $productos_disponibles) {
        echo "<script>alert('-{$_SESSION['nombre_usuario']} {$_SESSION['apellido_usuario']}-  La Cantidad a Apartar NO! debe Ser Mayor a la Disponible!! ');window.location='../descriProd.php?id=$id#ir';exit();</script>";
        exit();
    } elseif ($cantidadApartada <= 0) {
        echo "<script>alert('-{$_SESSION['nombre_usuario']} {$_SESSION['apellido_usuario']}-  La Cantidad a Apartar NO! debe Ser 0 o Negativa ');window.location='../descriProd.php?id=$id#ir';exit()</script>";
        exit();
    }
}
...
if ($resul1 >= 1) {
    ...
    $sqlLog1 = str_replace("'", "", $valida2);
    $sqlLog2 = str_replace("'", "", $detalle_apartado1);
...
if ($resul1 == 0) {
    ...
    if ($result2 == true) {
        ...
        $sqlLog1 = str_replace("'", "", $apartado1);
        $sqlLog2 = str_replace("'", "", $sql1);
        $sqlLog3 = str_replace("'", "", $detalle_apartado1);
