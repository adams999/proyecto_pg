...
<body>
    ...
    <article class="row">
        ...
        $sql="SELECT * FROM proveedores where estatus=1 ORDER BY id_proveedor ASC";
     ...
     if(isset($_GET['ini'])){
        $ini=$_GET['ini'];
        $fin=$_GET['fin'];
        $sql="SELECT * FROM proveedores where estatus=1 ORDER BY id_proveedor ASC limit 10 offset $ini";
    }else{
        $sql="SELECT * FROM proveedores where estatus=1 ORDER BY id_proveedor ASC limit 10 offset 0";
    }
            ...
            <table class="table table-bordered table-hover table-striped" align="center">
                ...
                <tr class="parrafo " align="center">
                    ...
                    <td class="titulo" width="5%">
                        <a href="modificarProveedor.php?id='.$arreglo['id_proveedor'].'#ir">
...
if (@$query2==false or @$query==false) {
...
for($c=1;$c<=$pag;$c++){
    $ini=$c*10-10;
    $fin=$ini+9;
    echo ' <a href="listaProveedores.php?ini='.$ini.'&fin='.$fin.'#ir">'.$c.'</a>&nbsp;&nbsp;';
}
echo '</div>';
?>           
