...
if ($nomb_banc_usua == null or $tipo_cuen_usua == null or $titu_cuen_usua == null or $cedu_cuen_usua == null or $num_cuen_usua == null) {
	echo "<script>alert('Por Favor Completa Todos Los Campos De La Tienda Ya Que Son Obligatorios!!');
				window.location='../registrarCuentaUsuario.php';exit();</script>";
	exit();
} else {
	...
	if ($resul1 >= 1) {
		echo "<script>alert('ATENCIÓN: El Número de Cuenta  - $num_cuen_usua -  ya Existe Por Favor ingresar un Número de Cuenta Válido');window.location='../registrarCuentaUsuario.php#ir';exit();</script>";
		exit();
	}
}
