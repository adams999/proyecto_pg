...
if ($nomb_banc_empr == null or $tipo_cuen_empr == null or $titu_cuen_empr == null or $cedu_cuen_empr == null or $num_cuen_empr == null) {
	echo "<script>alert('Por Favor Completa Todos Los Campos De La Tienda Ya Que Son Obligatorios!!');
				window.location='../registroBancoEmpresa.php';exit();</script>";
	exit();
} else {
	...
	if ($resul1 >= 1) {
		echo "<script>alert('ATENCIÓN: El Número de Cuenta  - $num_cuen_empr -  ya Existe Por Favor ingresar un Número de Cuenta Válido');window.location='../registroBancoEmpresa.php#ir';exit();</script>";
		exit();
	}
}
