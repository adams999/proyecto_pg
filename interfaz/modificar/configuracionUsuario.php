...
if ($contraseña_antigua == null) {
	echo "<script>alert('ATENCIÓN Debes Ingresar Tu Contraseña Actual Para Poder Ejecutar Cualquier Cambio');window.location='../configUsuario.php#ir';exit()</script>";
	exit();
}
...
elseif ($contraseña_antigua != null and $contraseña_antigua != $contraUsuario) {
	echo "<script>alert('ATENCIÓN. La Contraseña Actual No Coincide con la Registrada Por Favor Ingresar Contraseña Valida... Para Poder Proceder Con Los Cambios');
			window.location='../configUsuario.php#ir';exit();</script>";
	exit();
}
...
elseif ($contraseña_antigua != null and $contraseña_nueva != null and $contraseña_antigua == $contraseña_nueva) {
	echo "<script>alert('ATENCIÓN La Contraseña Nueva no Debe ser Igual a la Ya Establecida!!');window.location='../configUsuario.php#ir';exit()</script>";
	exit();
} elseif ($contraseña_nueva != $repite_contraseña) {
	echo "<script>alert('ATENCIÓN La Contraseña Nueva Debes Ingresarla 2 Veces En Los Campos Correspondentes... Y En Este Caso No Coinciden Por Favor Ingresa Contraseña Correcta Para Que Puedan Surgir Los Cambios');window.location='../configUsuario.php#ir';exit()</script>";
	exit();
}
...
if ($contraseña_antigua != null and $contraseña_antigua == $contraUsuario and $telefono_hogar != null and $pregunta_seguridad != null and $telefono_personal != null and $respuesta_seguridad != null) {
	...
	if (isset($result3)) {
		...
		window.location='../configUsuario.php';exit();</script>";
		exit();
	}
