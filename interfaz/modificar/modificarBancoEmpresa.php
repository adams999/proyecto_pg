...
session_start();
if ($_SESSION['correo_usuarioA']) { } else {
...
if ($nomb_banc_empr == null or $tipo_cuen_empr == null or $titu_cuen_empr == null or $cedu_cuen_empr == null or $num_cuen_empr == null) {
	echo "<script>alert('POR FAVOR Completa Todos Los Campos del Banco para proceder con el Registro!!');
				window.location='../ModificarBancoEmpresa.php?id=$id';exit();</script>";
	exit();
} else {
	...
	if ($resul1 >= 1) {
		echo "<script>alert('ATENCIÓN: El Número de Cuenta  - $num_cuen_empr -  ya Existe Por Favor ingresar un numero de cuenta Válido');window.location='../ModificarBancoEmpresa.php?id=$id#ir';exit();</script>";
		exit();
	}
}
